...
function buildPasswordPreservationKey(values: {
  ...
  return JSON.stringify({
    ...
    remoteRoot: values.remoteRoot ?? "cli-switch-sync",
...
export function WebdavSyncSection({
  ...
  const [form, setForm] = useState(() => ({
    ...
    remoteRoot: config?.remoteRoot ?? "cli-switch-sync",
  ...
  const [s3RemoteRoot, setS3RemoteRoot] = useState(
    s3Config?.remoteRoot ?? "cli-switch-sync",
  );
  ...
  useEffect(() => {
    ...
    setForm(() => {
      ...
      const nextRemoteRoot = config.remoteRoot ?? "cli-switch-sync";
  ...
  useEffect(() => {
    ...
    setS3RemoteRoot(s3Config.remoteRoot ?? "cli-switch-sync");
  ...
  const buildSettings = useCallback((): WebDavSyncSettings | null => {
    ...
    return {
      ...
      remoteRoot: form.remoteRoot.trim() || "cli-switch-sync",
  ...
  const buildS3Settings = useCallback((): S3SyncSettings => {
    return {
      ...
      remoteRoot: s3RemoteRoot.trim() || "cli-switch-sync",
  ...
  const currentRemotePath = `/${form.remoteRoot.trim() || "cli-switch-sync"}/v2/db-v6/${form.profile.trim() || "default"}`;
  const currentS3RemotePath = `${s3Bucket.trim() || "bucket"}/${s3RemoteRoot.trim() || "cli-switch-sync"}/v2/db-v6/${s3Profile.trim() || "default"}`;
    ...
    <section className="space-y-4">
      ...
      {syncType === "webdav" && (
        <div className="space-y-4 rounded-lg border border-border bg-muted/40 p-6">
          ...
          {showAutoSyncError && (
            <div className="rounded-lg border border-negative/70 bg-negative/10/80 px-3 py-2 text-xs text-negative dark:border-negative/50 dark:bg-negative/10/30 dark:text-negative">
              <p className="font-medium">
                {t("settings.webdavSync.autoSyncLastErrorTitle")}
              </p>
              <p className="mt-1 break-all whitespace-pre-wrap">{lastError}</p>
              <p className="mt-1 text-[11px] text-negative/90 dark:text-negative/80">
          ...
          <div className="flex flex-wrap items-center gap-3 pt-2">
            ...
            {dirty && (
              <span className="inline-flex items-center gap-1.5 text-xs text-warning dark:text-warning animate-in fade-in duration-200">
                <span className="h-1.5 w-1.5 rounded-full bg-warning dark:bg-warning" />
                {t("settings.webdavSync.unsaved")}
              </span>
            )}
            {!dirty && justSaved && (
              <span className="inline-flex items-center gap-1.5 text-xs text-positive dark:text-positive animate-in fade-in duration-200">
